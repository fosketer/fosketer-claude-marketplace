...
4. Include in response header alongside findings:
   ```json
   { "dimension": "testing", "score": <score>, "raw_penalty": <raw>, "summary": {...}, "findings": [...] }
   ```
5. Persist findings to `SCAN_REPORTS_DIR/YYYY-MM-DD-testing.json` (overwrite if same date exists)
