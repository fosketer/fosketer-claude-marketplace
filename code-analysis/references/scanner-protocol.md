# Scanner Protocol — Shared by All Scan Skills
...
3. ID = {DIM}-{file_hash6}-{title_hash4}

**Why title_hash instead of line_bucket:** Line numbers shift when code is refactored, breaking carry-forward ID matching and causing false "resolved" findings. Title hashes are stable across code movement.
...
**Carry-forward re-scan** (PREVIOUS_FINDINGS provided): carried-forward findings ALWAYS keep their existing ID. When a new finding collides with a carried-forward ID:
...
- Initial `/analyze-codebase` scans pass CHANGED_FILES=null (acceptable — no previous findings)
- If PREVIOUS_FINDINGS has >30 findings and CHANGED_FILES is null, the scanner MAY skip
  Phase 1 verification and carry all findings forward tentatively. Set `unverified` in
  carry_forward_summary to the count of tentatively carried findings.
